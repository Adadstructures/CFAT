...
if section == "Prediction":
    ...
    with col2:
        ...
        fal = st.number_input("Aluminium Yield Strength fal (MPa)", 70.0, 566.0, 240.0)
...
# ==================================================================
    ...
    col1, col2 = st.columns(2)
    with col1:
        target_load = st.number_input("Target Load Capacity (kN)", min_value=80.0, value=4000.0, step=50.0)
        height = st.number_input("Column Height (mm)", 114.0, 1620.0, 800.0)
    with col2:
        max_cost   = st.number_input("Maximum Allowed Cost (USD)", 0.0, 10000.0, 1000.0)
        max_carbon = st.number_input("Maximum Allowed Carbon (kg CO₂e)", 0.0, 10000.0, 1500.0)
    ...
    if st.button("Run NSGA-II Optimisation"):
        ...
        if res.F.size > 0:
            F = res.F
            # Choose balanced solution (closest to origin in normalised space)
            norm = F / F.max(axis=0)
            ...
            mass_c_opt = vol_c_opt * density['Concrete']
            mass_a_opt = vol_a_opt * density['Aluminium']
            ...
            st.error("No feasible solution found. Try increasing target tolerances or bounds.")

# ------------------------------------------------------------------
# Notes & References (exactly as you had)
# ------------------------------------------------------------------
st.markdown("""
    **Notes**: 
...
# ------------------------------------------------------------------
# YOUR ORIGINAL FOOTER — UNTOUCHED
# ------------------------------------------------------------------
footer = """
<style>
.footer {
...
<div class="footer">
    <p>© 2025 My Streamlit App. All rights reserved. | Temitope E. Dada, Silas E. Oluwadahunsi, Oluwafemi Omotayo, Charles K.S. Moy and Yao Sun | For Queries: <a href="mailto:topeemmanueldada@yahoo.co.uk">topeemmanueldada@yahoo.co.uk</a></p>
</div>
"""
